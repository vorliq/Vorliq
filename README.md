...
The signed snapshot archive stores the latest 30 public signed snapshot records and is refreshed during deploy plus daily at 04:00 Europe/London. Use `node tools/verify_snapshot_archive.js https://vorliq.org` to verify the latest archive, `--list` to list recent archives, and `--hash SNAPSHOT_HASH` to verify a specific archived snapshot. Use `node tools/bootstrap_verify_node.js https://vorliq.org` for a read-only bootstrap report before trusting a node as a source of public state.
